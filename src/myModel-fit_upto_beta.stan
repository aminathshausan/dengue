...
data {
    int<lower=1> n_obs; //number of observed measurements 
     ...
     real ts[n_obs];    //time points of observed measurements
     real A;
     real sigma;
     real<lower =0> y_hat[n_obs];  //observed viremia measurements only
     int<lower =1> n_pred;        // number of predicted values from posterior, computed in the generated quantities block 
    real  t_pred[n_pred];        //time span at which posterior predictions are made
...
transformed parameters {
              ...
              //compute solutions only at the observed time points (ts)
                 y = integrate_ode_rk45(ode, y0, t0, ts, params, x_r, x_i, 1e-6, 1e-6, 1e6);
                 
             }
            //add machine precision to vector y
               for (t in 1:n_obs) {
                  for (z in 1:6){
...
generated quantities {
       ...
       //compute predictions including incubation period (t_pred), without measurement variablity
         y_pred = integrate_ode_rk45(ode, y0, t0, t_pred, params, x_r, x_i, 1e-6, 1e-6, 1e6);
     
       }
       
      //compute predictions using measurement varaiability (std)
      for (t in 1:n_pred) {
         y_ppc[t] = lognormal_rng(log(y_pred[, 5][t]+ 10*1e-6), std);
