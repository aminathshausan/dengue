...
#create prediction time(s) matrix by adding IP of 5 days to each observed time(s) vector. 
#This corresponds to a  reflection of observed time span about the first observed time point.
t_pred <- matrix(data =NA, ncol =48, nrow = 10); #predicted times
t0 <- matrix(data = NA,  ncol = 48, nrow =1);   #initial times of ODE
for (i in 1:48) {
  t_pred[,i] <- (times_cl1[,i][1]-5):times_cl1[,i][5]
  t0[i] <- times_cl1[,i][1]-6
}
...
stan_data <- list(J = J,                     #number of patients
                  n_obs = 5,                 #number of observed measurements per patient
                  n_pred = 10,                #number of predicted measurements (from generated quantities block)
...
test <- stan("myModel-fit_censored_noPool.stan",
             data = stan_data,
             chains = 1, iter = 100, 
...
save.image(file = '../results/p1_to_p10_noPool_with_V0.RData')
#-------------------------------------------------------
